        ...
        if (bookId !== null && data.isOpen === true) {
          ...
          if (responseBook.data.status === 200) {
            setBook({
              ...
              categoryId: responseCategories.data.data.find(
                (e) => e.id === responseBook.data.data.categoryId,
              ),
            });
          }
        }
                ...
                {p.type === 'select' ? (
                  <Autocomplete
                    ...
                    onChange={(event, newValue) =>
                      setBook({ ...book, [p.id]: newValue })
                    }
                    renderInput={(params) => (
                      <TextField {...params} fullWidth label={p.label} />
                    )}
                  />
                ) : p.type === 'date' ? (

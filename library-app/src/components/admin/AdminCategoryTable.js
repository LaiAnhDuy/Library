...
import { apiDeleteCategory, apiGetAllCategory, apiUpdateCategory } from '../../services/Category';
...
export default function AdminCategoryTable(props) {
    ...
    const handleUpdateCategory = async(index) => {
        ...
        try {
            ...
            if (response.data.status === 200) {
                dataAlert = { ...dataAlert, severity: 'success', isOpen: true, message: 'Cập nhật thành công' };
                setUpdate(!update);
            }
            else dataAlert = { ...dataAlert, severity: 'error', isOpen: true, message: 'Cập nhật thất bại' };
    ...
    const handleDeleteCategory = async(index) => {
        ...
        try {
            ...
            dispatch(closeLoadingModal());
            if (response.data.status === 200) {
                dataAlert = { ...dataAlert, severity: 'success', isOpen: true, message: "Xóa thành công" };
                setUpdate(!update);
            }
            else dataAlert = { ...dataAlert, severity: 'error', isOpen: true, message: "Xóa thất bại" };
            dispatch(openAlertModal(dataAlert));
        } catch(err) {
            console.log(err);
            dataAlert = { ...dataAlert, severity: 'error', isOpen: true, message: "Xóa thất bại" };
            dispatch(closeLoadingModal());
            dispatch(openAlertModal(dataAlert));
        }
    }
            ...
            <Paper sx={{ width: '100%', mb: 2 }}>
                <TableContainer>
                    ...
                    >
                        ...
                        />
                        <TableBody>
                            {categories.slice((page) * rowsPerPage, (page + 1) * rowsPerPage)
                                ?.map((row, ii) => {
                                    return (
                                        <TableRow
                                            hover
                                                ...
                                                return p.id === 'action' ? (
                                                    <TableCell key={index}>
                                                        <Grid sx={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                                                            <EditIcon color='primary' onClick={() => handleUpdateCategory(ii)}/>
                                                            <DeleteIcon color='error' onClick={() => handleDeleteCategory(ii)}/>
                                                        </Grid>
                                                    </TableCell>
                                                ) : p.id === 'id' ? (

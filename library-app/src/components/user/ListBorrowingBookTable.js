        ...
        if (response.data.status === 200) {
          let orderDatas = response.data.data.map((p) => {
            return {
              ...
              book: p.book.title,
              borrowDate:
                p.borrowDate === null ? null : formatDate(p.borrowDate),
              dueDate: p.dueDate === null ? null : formatDate(p.dueDate),
              returnedDate:
                p.returnedDate === null ? null : formatDate(p.returnedDate),
            };
          });
          setCount(orderDatas.length);
                          ...
                          <TableCell key={index}>
                            ...
                            >
                              <EditIcon
                                color="primary"
                                onClick={() =>
                                  dispatch(openOrderModal(row.code))
                                }
                              />
